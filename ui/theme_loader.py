...
from utils.paths import get_resource_path # <--- IMPORTAR
...
def load_theme(theme_filename: str = "theme_light.json") -> str:
    ...
    Args:
        theme_filename: Nombre del archivo (ej: 'theme_light.json')
                       o ruta relativa 'config/theme_light.json'
        
    Returns:
        String con el stylesheet QSS
    """
    # Si viene solo el nombre, asumimos que está en config/
    if "/" not in theme_filename and "\\" not in theme_filename:
        relative_path = Path("config") / theme_filename
    else:
